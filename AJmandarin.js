      ...
      const show_next_word = (position, that) => {
        if (position < n_words) {
          ...
          for (var i = 0; i < n_words; i++) {
            ...
            if (i === position) {
              stimulus += `<span style="visibility: visible;">${displayed_word}</span> `;
            } else {
              var hidden_word = '';
              for (var j = 0; j < displayed_word.length; j++) {
                hidden_word += `<span style="visibility: hidden;">${displayed_word[j]}</span>`;
              }
              stimulus += `<span>${hidden_word}</span> `;
            }
          }
